...
# from model.listOperationsWagon import operation
import pandas as pd
import os
import pytz
...
def getTripsWagon():
    ...
    # dropear columnas
    df_travels = df_travels.drop(['travelId', 'status'], axis=1)
    ...
    df_travels['datetimeMina'] = df_travels['createdAt'].dt.tz_localize('UTC').dt.tz_convert('America/Lima') + pd.Timedelta(hours=7)
    ...
    df_trips['month'] = df_trips['nro_month'].apply(lambda x: datetime(2020, x, 1).strftime('%B'))
...
def getOperationsWagon():
    ...
    df['datetimeMina'] = df['createdAt'].dt.tz_localize('UTC').dt.tz_convert('America/Lima') + pd.Timedelta(hours=7)
