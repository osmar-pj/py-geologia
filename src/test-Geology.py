...
df_reportes['level'] = df_reportes['level'].astype('Int64')
...
df_reportes['cod_despacho'].fillna('--', inplace=True)
...
df_reportes['date_abas'].fillna('2023-12-31', inplace=True)
df_reportes['date_abas'] = pd.to_datetime(df_reportes['date_abas'])
...
df_reportes['date'] = pd.to_datetime(df_reportes['date'])
df_reportes['timestamp'] = df_reportes['date'].apply(lambda x: datetime.strptime(x.strftime('%d/%m/%Y'), '%d/%m/%Y').timestamp()).astype(int)
...
df_reportes['week'] = df_reportes['date'].dt.isocalendar().week
...
df_reportes['vagones'] = df_reportes.apply(lambda x: round(x['tonh']/8) if (x['mining'] == 'UCHUCCHACUA' and x['vagones'] == 0) else x['vagones'], axis=1)
df_reportes['carriage'] = df_reportes['vagones'].apply(lambda x: 'Vagones' if x > 0 else 'Camion')
df_main = df_reportes.copy()
df_main.drop(['columna', 'company', 'cod_muestra3', 'ley_cancha_castigada', 'ley_stope', 'ley_inicial', 'labor', 'ley_original', 'ley_cancha2', 'ley_cancha3'], axis=1, inplace=True)
...
df_reportes.to_csv('reportes.csv', index=False)
...
# trips = db['listtrips']
# df_trips = pd.DataFrame(trips.find())
